...
- [env-versions-pinned] (ai, should-fix) Dependencies are pinned to exact versions, not just named
...
**No secrets or credentials are committed** (`no-committed-secrets`)
- [no-committed-secrets] (deterministic, must-fix, pass_when=absent) No secrets or credentials are committed
    evidence — any: .env / .Renviron committed to the repo | private key files (id_rsa, *.pem, *.ppk) | hard-coded API keys, tokens or passwords (e.g. api_key=, secret=, token=, password=) | cloud credentials (aws credentials file, .httr-oauth, service-account .json)
